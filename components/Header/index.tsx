...
const Header = () => {
  ...
  return (
    <>
      <header
        className={`header left-0 top-0 flex w-full items-center border-b border-b-body-color/50 bg-dark ${
          sticky
        ...
        <div className="w-full">
          <div className="relative flex items-center justify-between">
            ...
            <>
              <nav
                id="navbarCollapse"
                className={`navbar absolute right-0 z-30 w-[250px] rounded border-[.5px] border-body-color/50 bg-white px-6 py-4 duration-300 dark:border-body-color/20  lg:visible lg:static lg:w-auto lg:border-none lg:p-0 lg:opacity-100 dark:lg:bg-transparent ${
                  navbarOpen ? "visible top-full opacity-100 dark:bg-dark" : "invisible top-[120%] opacity-0"
                }`}>
                  ...
                  {menuData.map((menuItem, index) => (
                    <li key={menuItem.id} className="group relative">
                      {menuItem.path ? (
                        <Link
                          href={menuItem.path}
                          className={`flex py-2 text-base text-dark group-hover:opacity-70 dark:text-white lg:mr-0 lg:inline-flex lg:px-0 lg:py-4`}>
                          {menuItem.title}
                        </Link>
                      ) : (
                        <>
                          <a
                            ...
                            className="flex cursor-pointer items-center justify-between py-2 text-base text-dark group-hover:opacity-70 dark:text-white lg:mr-0 lg:inline-flex lg:px-0 lg:py-4">
                            {menuItem.title}
                            <span className="pl-3">
